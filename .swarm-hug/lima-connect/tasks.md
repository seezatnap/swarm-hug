...
- [x] (#9) Execute `docker --context <ctx> exec -it <container> <shell>` defaulting to `bash -l` (blocked by #3, #6, #8) (A)
...
## Testing
- [x] (#11) Run and document manual test scenarios from PRD (single VM+container auto-connect, multiple VMs prompt, no VMs error, VM with no containers error) (blocked by #9, #10) (A)
