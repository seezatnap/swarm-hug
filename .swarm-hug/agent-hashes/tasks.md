...
- [A] (#4) Update `src/runner.rs` to create `RunContext` at the start of `run_sprint()`, pass it to all worktree creation and cleanup calls, update sprint branch creation to use `ctx.sprint_branch()`, log the run hash at sprint start for visibility, and update all call sites that previously passed project string to pass `RunContext` instead [5 pts] (blocked by #2, #3)

## Assignments Removal

- [ ] (#5) Delete `src/team/assignments.rs` module entirely; remove `assignments` module export and `ASSIGNMENTS_FILE` constant from `src/team/mod.rs`; remove all `release_assignments_for_project()` calls and assignment checking logic from `src/runner.rs`; delete assignment-related tests [5 pts] (blocked by #4)
