...
- [A] (#4) Reorder `run_sprint()` in `src/runner.rs` to create sprint branch/worktree FIRST before any file writes: determine sprint number using `peek_next_sprint()`, compute sprint branch name, then call `create_feature_worktree_in()` before loading/saving any state files [5 pts] (blocked by #1)
