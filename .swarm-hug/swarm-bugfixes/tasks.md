...
- [x] (#10) In split source/target runs, pass the canonical team directory (`.swarm-hug/<team>`) to agent engines instead of the namespaced runtime directory (`.swarm-hug/<team>/runs/<target>`), so prompt-derived paths for `team-state.json` and sprint worktrees resolve correctly. (B)
