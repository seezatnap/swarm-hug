...
- [A] (#2) Add a strict `Critical Rules` section in `prompts/merge_agent.md` that explicitly bans `git merge --squash`, `git cherry-pick`, `git diff | git apply`, and `git rebase`, and requires conflict resolution to stay inside the original `git merge --no-ff` path [5 pts] (blocked by #1)
- [A] (#3) Add merge-state safety and verification instructions in `prompts/merge_agent.md`: require `.git/MERGE_HEAD` before any manual commit, define recovery by restarting `git merge --no-ff` if `MERGE_HEAD` is lost, and require post-commit verification that `git rev-parse HEAD^2` succeeds [5 pts] (blocked by #1)
