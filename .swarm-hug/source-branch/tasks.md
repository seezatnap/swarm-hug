...
- [A] (#3) Refactor branch creation paths in `src/runner.rs` and `src/worktree/` so feature/sprint branches are created from `source_branch` (not `target_branch`), including `create_feature_worktree_in()` / `create_feature_branch_in()` call chains [5 pts] (blocked by #2)
...
- [A] (#5) Update `src/commands/run.rs` TUI re-invocation to pass `--source-branch` through to subprocesses and keep argument behavior consistent with non-TUI execution for all supported flag combinations [5 pts] (blocked by #2)
