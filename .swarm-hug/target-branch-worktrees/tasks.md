...
- [A] (#2) Implement target branch worktree resolution by parsing `git worktree list --porcelain` output to locate worktrees for `refs/heads/<target-branch>`, returning worktree path if found or None if not [5 pts] (blocked by #1)
