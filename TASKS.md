...
- [x] Merge branch `agent-process-lifecycle-sprint-3` into `main` and resolve any conflicts
...
- [x] (#8) Add integration tests for subprocess cleanup: test `test_engine_timeout_no_zombie` that spawns engine with short timeout and verifies no zombie processes remain; test `test_shutdown_kills_subprocess` that sets shutdown flag and verifies subprocess terminates within 500ms [5 pts] (blocked by #1, #6, #7)
